...
author_hdf5_file=h5py.File('/fileserver/nmec-handwriting/splits/author_nmec_cropped_bin_uint8_train.hdf5')
hdf5_file=h5py.File('/fileserver/nmec-handwriting/splits/flat_nmec_cropped_bin_uint8_train.hdf5')

author_hdf5_val_file=h5py.File('/fileserver/nmec-handwriting/splits/author_nmec_cropped_bin_uint8_val.hdf5')
hdf5_val_file=h5py.File('/fileserver/nmec-handwriting/splits/flat_nmec_cropped_bin_uint8_val.hdf5')
...
def get_batch( author_hdf5_file, author_ids, shingle_size=(56,56), data_size=32 ):
...
def get_pbatch( author_hdf5_file, author_ids, data_size=32*1000, shingle_dim = (56,56) ):
...
if True:
    for batch_iter in xrange(total_iters):
        ...
        if (batch_iter % 25)==0:
            (X_val, Y_val) = get_pbatch( author_hdf5_val_file, author_ids, data_size=32*100, shingle_dim = shingle_dim )
        ...
        if (batch_iter % 20)==0 and batch_iter != 0:
            model.save_weights('fielnet56-nmec5doc.hdf5', overwrite=True)
