...
def create_model(num_authors, shingle_dim, lr):
    model = Sequential()
    model.add(Convolution2D(48, 14, 14,
                        border_mode='full',
...
def run_model(hdf5_file, num_authors, num_forms_per_author, shingle_dim, num_iters=10, batch_size=32,
              use_form=False, lr=0.03, loadweights=None):
    # Create Keras model
    ...
    for i in range(num_iters):
        ...
        model.fit(X_train, Y_train, batch_size=batch_size, nb_epoch=1,
                  show_accuracy=True, verbose=1, validation_data=(X_test, Y_test)) # verbose=1
...
def main():
    ...
    parser.add_option("--batch_size", dest="batch_size", type=int, default=16, help="Iteration Batch Size")
    ...
    parser.add_option("--weights", dest="loadweights", default=None, help="Load the weights in")
    (options, args) = parser.parse_args()

    shingle_str = options.shingle_dim.split(',')
    shingle_dim = (int(shingle_str[0]), int(shingle_str[1]))

    print "Loading in weights to load: "+str(options.loadweights)
    
    run_model(options.filename, options.num_authors, options.num_forms_per_author,shingle_dim, options.num_iters, options.batch_size, options.use_form, options.lr, loadweights=options.loadweights)
