...
class MiniBatcher:
    ...
    def __init__(self, hdf5_file, input_keys, item_getter,
        ...
        if round(1e6 * (train_pct + test_pct + val_pct)) != 1e6:
            raise ValueError('Train(%f)+ Test(%f) + Validation(%f) set percentatages must add to 1.0' % (
        ...
        for (id_str, id_count) in author_counts.items():
            if id_count >= min_fragments:
                ...
                logger.debug("Skipped author {0}, with {1} fragments (min_fragments={2})".format(id_str, id_count,
                                                                                                 min_fragments))
